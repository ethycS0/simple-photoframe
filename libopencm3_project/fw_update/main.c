...
int main(void) {
        ...
        if(response[0] != session_ACK) {
                printf("Error initializing Session. Please Restart.\n");
                return -1;
        }

        // packet_t packet;
        // while(total_packets_bytes--) {
        //         unsigned char data[16];
        //         size_t items_read = fread(data, sizeof(unsigned char), 16, firmware);
        //         if(items_read < 16) {
        //                 if(feof(firmware)) {
        //                         printf("Done Reading Firmware\n");
        //                         // Add Padding
        //                 } else {
