...
void usart2_isr(void) {
        const bool overrun_occured =
            usart_get_flag(USART2, USART_FLAG_ORE) == 1;
...
bool uart_timeout(uint64_t timeout) {
        uint64_t end = system_get_ticks() + timeout;
        while (!uart_data_available()) {
                if (system_get_ticks() >= end) {
                        return false;
                }
        }
        return true;
...
bool uart_data_available(void) { return !ringbuffer_empty(&rb); }
